...
Current-position Human Lens results are also runtime state, separate from the
optional `MoveAnalysis.human` enrichment supported for stored analysis and Coach facts. They
are associated with an exact FEN, Maia model and target Elo. A Maia-created branch
edge may store `{ kind: "maia", targetElo, probability }` as source evidence; it
does not gain Accuracy, classification or an engine score from that metadata.

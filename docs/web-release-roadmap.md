...
本计划承接此前两个任务的实现，以及 [9 月 5 日审计](/Users/ice/Code/chess-review/docs/audits/2026-09-05-web-launch-and-art-direction.md)。依据用户最新要求，暂时保留当前棋子、棋盘颜色、全站配色、品牌与 V3 着法图标。本轮聚焦现有问题、UI/UX、布局和上线业务流程；此前艺术方向草案保留参考。
...
修复方向：压缩标题和重复说明，先解决主操作可见性；按实际可用空间分配棋盘与内容列，保留 timeline 在右侧 Game Summary 中。改善字号、行高、触控区域、键盘焦点与阅读顺序，继续使用现有棋子和配色。

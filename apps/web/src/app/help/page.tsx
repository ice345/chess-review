  ...
  return <main className="page-scroll utility-page"><AppHeader />
    ...
    <article className="help-content">
      ...
      <section id="first-review"><h2>Your first review</h2><ol>
        ...
        <li>Visit <Link href="/training">Training</Link> to find recurring decisions across analyzed games. Add a task, start its source position, and say what each review was worth. Continue picks up whatever the schedule is waiting for, otherwise the first position you have not reviewed yet.</li>
      ...
      <section id="capabilities"><h2>What is available</h2>
        ...
        <p>Stockfish supplies objective evaluations. Maia predicts human choices; it is not an objective evaluator. The Coach explains structured evidence and cannot override it. A position-review count is a ledger of the source positions you have gone through, not a score. Training mastery is separate: each position follows a due date, and it only advances when you produce the move unaided on a day the position came round again. Neither number is an Elo measurement.</p>
        <p>FEN studies have no game history, so use Engine Lab for position analysis. PGN is required for whole-game Accuracy and move-review history. Engine Lab also carries the Opening Explorer, which looks up what other players do from the current position in a public lichess.org database, and Tablebase, which reports the proven result for positions of at most seven pieces.</p>
      </section>
      ...
      <section id="your-data"><h2>Your data and connected services</h2>
        <dl className="data-destinations">
          ...
          <div><dt>Tablebase</dt><dd>When you open the Tablebase panel in Engine Lab for a position of at most seven pieces, that position is sent to the public Syzygy tables through this website&rsquo;s server, and the answer is cached in this browser. Only the position leaves this machine. A position above the coverage limit is never sent, and no engine evaluation is presented as a proven result. Clearing the analysis cache also clears that cache.</dd></div>

...
describe("review session progress", () => {
  it("reports an early finish as zero moments viewed, never as a finished review", () => {
    ...
    expect(reviewCompletionHeadline(counts)).toBe("0 of 3 key moments viewed");
    expect(reviewCompletionDetail(counts)).toBe("3 moments are still unseen; they stay available in Moves and Study.");
  });
  ...
  it("counts repeated visits once", () => {
    ...
    expect(reviewCompletionHeadline(counts)).toBe("2 of 3 key moments viewed");
  });
  ...
  it("reports full browsing as browsing, not as learning", () => {
    ...
    expect(reviewCompletionHeadline(counts)).toBe("All 3 key moments viewed");
    expect(reviewCompletionDetail(counts)).toBe("Viewing a position is not the same as solving it; practice results are counted separately.");
  });
  ...
  it("allows finishing a game with no key moments without inventing a count", () => {
    ...
    expect(reviewCompletionHeadline(counts)).toBe("No key moment crossed the thresholds");
    expect(reviewCompletionDetail(counts)).toBe("This game had no swing large enough for a guided moment.");
  });

  it("describes a single moment in the singular", () => {
    const counts = reviewSessionCounts(session({ seen: [3] }), moments.slice(0, 1));

    expect(reviewCompletionHeadline(counts)).toBe("The only key moment was viewed");
    expect(reviewCompletionDetail(counts)).toBe("Viewing a position is not the same as solving it; practice results are counted separately.");
  });

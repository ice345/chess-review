...
describe("run diagnostics copy", () => {
  it("states what the run did, including that nothing completed", () => {
    expect(runDiagnosticsLine(summary)).toBe("21 searches · 1.8 s to the first · slowest 6.1 s · 3 engines started · over 12.4 s");
    expect(runDiagnosticsLine({ durationMs: 40_000, searches: 0, workerSpawns: 1, cancelled: 1, failures: 0 })).toBe("0 searches · no completed search · cancelled · over 40.0 s");
    expect(runDiagnosticsLine({ durationMs: 900, searches: 1, workerSpawns: 1, cancelled: 0, failures: 0, firstSearchMs: 800 })).toBe("1 search · 0.8 s to the first · over 0.9 s");
  });
